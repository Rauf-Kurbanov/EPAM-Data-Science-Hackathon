    ...
    def predict_batch(self, batch, top_n=1):
        ...
        for i in range(len(s)):
            ...
            for j in range(len(s[i])):
                predictions['text_tokens'] = d_tokens[i]
                predictions['span'] = d_tokens[i].slice(s[i][j], e[i][j] + 1).untokenize()
                predictions['score'] = score[i][j]
                predictions['start'] = s[i][j]
                predictions['end'] = e[i][j] + 1
